...
// formatted text.
func NewTextReporter(urlM map[string]*Metrics) Reporter {
...
func NewMultiplePlotReporter(rs *Results) Reporter {
	return func(w io.Writer) (err error) {
		_, err = fmt.Fprintf(w, plotsHead1)
		if err != nil {
			return err
		}
		...
		for _, result := range *rs {
			v, ok := resultsMap[result.Url]
			if ok {
				v.Add(&result)
			} else {
				var rs Results
				rs.Add(&result)
				resultsMap[result.Url] = &rs
			}
		}

		for k, _ := range resultsMap {
			_, err = fmt.Fprintf(w, plotsHead2, k)
			if err != nil {
				return err
			}
		}

		_, err = fmt.Fprintf(w, plotsHead3, asset(dygraphs), asset(html2canvas))
		if err != nil {
			return err
		}

		for k, v := range resultsMap {
			_, err = fmt.Fprintf(w, plotsTail1, k)
			if err != nil {
				return err
			}
			...
			_, err = fmt.Fprintf(w, plotsTail2, k)
			if err != nil {
				return err
			}
		}

		_, err = w.Write([]byte(plotsTail3))
		return err
	}
}

const (
	plotsHead1 = `<!doctype html>
<html>
<head>
  <title>Vegeta Plots</title>
</head>
<body>`
	plotsHead2 = `<div id="latencies_%s" style="font-family: Courier; width: 100%%; height: 600px"></div>`
	plotsHead3 = `<script>%s</script>
  <script>%s</script>
  <script>`
    plotsTail1 = `new Dygraph(
    document.getElementById("latencies_%s"),
    [`
	plotsTail2 = `],
    {
      title: 'Vegeta Plot_%s',
      labels: ['Seconds', 'ERR', 'OK'],
      ylabel: 'Latency (ms)',
      xlabel: 'Seconds elapsed',
      showRoller: true,
    ...
    plotsTail3 = `document.getElementById("download").addEventListener("click", function(e) {
    html2canvas(document.body, {background: "#fff"}).then(function(canvas) {
      var url = canvas.toDataURL('image/png').replace(/^data:image\/[^;]/, 'data:application/octet-stream');
      var a = document.createElement("a");
      a.setAttribute("download", "vegeta-plot.png");
